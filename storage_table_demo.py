...
resourcegroup_name = 'srp'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'srp'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
car.price = 24,000
table_service.insert_entity('itemstable', car)
print('Created entry for Suburu...\n')
...
coffee.price = 3.75
table_service.insert_entity('itemstable', coffee)
print('Created entry for McCafe...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'cars'", select='make,model,year,color,price')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffees'", select='brand,flavor,size,price')
for item in items:
